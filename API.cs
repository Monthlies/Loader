...
namespace Loader
{
    ...
    internal class OnProgramStart
    {
        ...
        public static void Initialize(string name, string aid, string secret, string version)
        {
            ...
            {
                MessageBox.Show("Invalid application information!", Name, MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show("Possible malicious activity detected!", OnProgramStart.Name, MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Warning);
                        Process.GetCurrentProcess().Kill();
                    }
                    ...
                    {
                        MessageBox.Show("Security error has been triggered!", Name, MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    switch (response[2])
                    {
                        case "success":
                            ...
                            {
                                MessageBox.Show("Application is in Developer Mode, bypassing integrity and update check!", Name, MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Warning);
                            ...
                            else
                            {
                                ...
                                {
                                    MessageBox.Show($"Update {ApplicationSettings.Version} available, redirecting to update!", Name, MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
                                ...
                                if (response[12] == "Enabled")
                                {
                                    ...
                                    {
                                        MessageBox.Show($"File has been tampered with, couldn't verify integrity!", Name, MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
                                        Process.GetCurrentProcess().Kill();
                                    }
                                }
                            }
                            ...
                            {
                                MessageBox.Show("Looks like this application is disabled, please try again later!", Name, MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
                                Process.GetCurrentProcess().Kill();
                            }
                            break;
                        case "binderror":
                ...
                {
                    MessageBox.Show(ex.Message, Name, (MessageBoxButtons)(MessageBoxButton)MessageBoxButtons.OK, (MessageBoxIcon)MessageBoxImage.Error);
                    Process.GetCurrentProcess().Kill();
                }
            }
        }
    ...
    internal class API
    {
        public static void Log(string username, string action)
        {
            ...
            {
                MessageBox.Show("Please initialize your application first!", OnProgramStart.Name, (MessageBoxButtons)MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
                Process.GetCurrentProcess().Kill();
            }
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                {
                    System.Windows.MessageBox.Show(ex.Message, OnProgramStart.Name, (System.Windows.MessageBoxButton)MessageBoxButton.OK, (System.Windows.MessageBoxImage)MessageBoxImage.Error);
                    Process.GetCurrentProcess().Kill();
                }
            }
        }
        ...
        public static bool AIOLogin(string AIO)
        {
            if (!Constants.Initialized)
            {
                System.Windows.MessageBox.Show("Please initialize your application first!", OnProgramStart.Name, (System.Windows.MessageBoxButton)MessageBoxButton.OK, (System.Windows.MessageBoxImage)MessageBoxImage.Error);
                Process.GetCurrentProcess().Kill();
            }
            if (string.IsNullOrWhiteSpace(AIO))
            {
                System.Windows.Forms.MessageBox.Show("Missing user login information!", ApplicationSettings.Name, (MessageBoxButtons)MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                try
                {
                    ...
                    if (response[0] != Constants.Token)
                    {
                        System.Windows.MessageBox.Show("Security error has been triggered!", OnProgramStart.Name, (System.Windows.MessageBoxButton)MessageBoxButton.OK, (System.Windows.MessageBoxImage)MessageBoxImage.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Security.MaliciousCheck(response[1]))
                    {
                        System.Windows.MessageBox.Show("Possible malicious activity detected!", OnProgramStart.Name, (System.Windows.MessageBoxButton)MessageBoxButton.OK, (System.Windows.MessageBoxImage)MessageBoxImage.Warning);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Constants.Breached)
                    {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "time_expired":
                            ...
                            return false;
                        case "hwid_updated":
                            System.Windows.MessageBox.Show("New machine has been binded, re-open the application!", ApplicationSettings.Name, (System.Windows.MessageBoxButton)MessageBoxButton.OK, (System.Windows.MessageBoxImage)MessageBoxImage.Information);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "invalid_hwid":
                            System.Windows.MessageBox.Show("This user is binded to another computer, please contact support!", ApplicationSettings.Name, (System.Windows.MessageBoxButton)MessageBoxButton.OK, (System.Windows.MessageBoxImage)MessageBoxImage.Error);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                    }
    ...
    internal class Security
    {
        ...
        {
            Random random = new Random();
            const string chars = "gd8JQ57nxXzLLMPrLylVhxoGnWGCFjO4knKTfRE6mVvdjug2NF/4aptAsZcdIGbAPmcx0O+ftU/KvMIjcfUnH3j+IMdhAW5OpoX3Mr98VgV8dxRQA2fCMcjRegFvvLivxAAjt3VaqMWZHKtPSuAXy0kkZRsc+DicpcY8E9+vWMHXa3jMdbPx4YES0p66GzhqLd/heA2zMvX8iWv4wK7S3QKIW/a9dD4ALZJpmcr9OOE=";
            return new string(Enumerable.Repeat(chars, length)
             .Select(s => s[random.Next(s.Length)]).ToArray());
        }
